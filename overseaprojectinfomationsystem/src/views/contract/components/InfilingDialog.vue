<template>
  <el-dialog v-model="dialogVisible" :destroy-on-close="true" size="450px" :title="`${dialogProps.title}`">
    <el-form ref="ruleFormRef" label-width="150px" label-suffix=" :" :rules="rules" :disabled="dialogProps.isView"
      ...
      <el-form-item label="收入合同来源" prop="incontractSource">
        <el-select v-model="dialogProps.row!.incontractSource" clearable>
          <el-option style="font-size: 20px;" v-for="item in incontractSource" :key="item.value" :label="item.label"
            :value="item.value" />
        </el-select>
      </el-form-item>
    ...
    <template #footer>
      ...
      <el-button style="font-size: 20px;height: auto;width:auto;" v-show="!dialogProps.isView" type="primary"
        @click="approvalInfilings">审批</el-button>
      <el-button style="font-size: 20px;height: auto;width:auto;" v-show="!dialogProps.isView" type="primary"
        @click="rejectInfilings">拒审</el-button>
    </template>
  </el-dialog>
</template>
...
// 审批
const approvalInfilings = async () => {
  await useHandleData(approvalInfiling, { id: dialogProps.value.row.id }, `审批`);
  dialogProps.value.getTableList!();
  dialogVisible.value = false;
};

// 拒审
const rejectInfilings = async () => {
  await useHandleData(rejectInfiling, { id: dialogProps.value.row.id }, `拒审`);
  dialogProps.value.getTableList!();
  dialogVisible.value = false;
};
