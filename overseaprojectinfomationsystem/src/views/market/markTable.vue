...
import { getMarkList, addMark, editMark, deleteMark, exportEventExcel } from "@/api/modules/market";
...
const columns = reactive<ColumnProps<Market.MarkTable>[]>([
  ...
  { prop: "note", label: "备注" },
  { prop: "key", label: "", search: { el: "input" }, isShow: false },
...
const ExportExcel = async () => {
  let res = await exportEventExcel();
  const blob = new Blob([res]);
  console.log(blob);
  const fileName = "市场营销.xlsx";
