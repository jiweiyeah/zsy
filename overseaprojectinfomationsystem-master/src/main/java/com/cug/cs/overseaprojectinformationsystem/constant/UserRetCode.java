...
public enum UserRetCode {
    ...
    USERORPASSWORD_ERROR               ("003001","用户名或密码不正确"),
    ...
    ADDRESS_ID_NOT_EXSITS                ("003201", "AddressId不存在"),
    GENERATE_VERIFY_CODE_FAIL            ("003202", "生成验证码失败"),
    REGISTER_ACTIVATION_MAIL_SEND_FAIL   ("003202", "生成验证码失败");
    
    private String code;
    private String message;
    
    UserRetCode(String code, String message) {
        this.code = code;
    ...
    public static String getMessage(String code) {
        for (UserRetCode s : UserRetCode.values()) {
            if (null == code)
                break;
